...
Figure *Polygon::copy() const {
    ...
    vector<Point *>::const_iterator it(_points.begin());
    vector<Point *>::const_iterator end(_points.end());
    while (it != end) {
...
Figure *Polygon::decode(char **message) {
    int size;
    Polygon *poly = new Polygon();

    *message = *message + PREFIX.size();
    std::sscanf(*message, ":%d", &size);
...
std::string Polygon::encode() const {
    string encodePolygon = PREFIX + ":" + std::to_string(_points.size());
    for (auto p : _points) {
...
bool Polygon::decodable(char *message) {
    return 0 == PREFIX.compare(0, PREFIX.size(), string(message));
}

void Polygon::show(ostream &stream) const {
    stream << "Polygon(";
    for (Point *p : _points) {
